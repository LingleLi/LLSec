...
def get_trained_model_and_tokenizer(model_name_or_path, mode, tokenizer_fast):
    ...
    if mode == "8bit-base":
        model = AutoModelForCausalLM.from_pretrained(model_name_or_path, return_dict=True, torch_dtype=torch.float16, device_map='cuda:0' if torch.cuda.is_available() else 'auto', trust_remote_code=True, load_in_8bit = True, use_flash_attention_2=True)
    elif mode == "base":
        model = AutoModelForCausalLM.from_pretrained(model_name_or_path, return_dict=True, torch_dtype=torch.float16, device_map='cuda:0' if torch.cuda.is_available() else 'auto', trust_remote_code=True, use_flash_attention_2=True)
    else:
