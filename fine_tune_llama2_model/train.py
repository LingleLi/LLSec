...
def train_model(model, last_checkpoint, tokenizer, train_dataset, eval_dataset, training_args, data_args, logger):
    ...
    if training_args.do_train:
        ...
        if data_args.model_dir is not None:
            trainer.save_model(f"{data_args.model_dir}/best_fine-tuned_model_{int(time.time())}")
        else:
            trainer.save_model(f"best_fine-tuned_model_{int(time.time())}")
