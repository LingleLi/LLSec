...
def relation_mining(defines, vars, rules, preliminaries, id_example):
    """
    r2到r3的转换
    1. 结合状态机
    2. 添加一些预定义的要素
    ...
    print(f"规则间关系识别用到的领域知识数：{len(list(set(knowledge_used)))}")

    return defines, vars, rules, explicit_relation_count, implicit_relation_count, explicit_relation, implicit_relation, relation, len(list(set(knowledge_used)))
...
def add_relation(rules, id_example):
    ...
    #         for t in rule['after']:
    #             if t not in keys:
    #                 new_after += [id1 for id1 in keys if t in id1]
    #             else:
    #                 new_after.append(t)
    #         rule['after'] = new_after
    #     if len(rule['before']) > 0:
    #         new_before = []
    #         for t in rule['before']:
    #             if t not in keys:
    #                 new_before += [id1 for id1 in keys if t in id1]
    #             else:
    #                 new_before.append(t)
    #         rule['before'] = new_before
