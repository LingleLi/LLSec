...
def generate_llm_chat_data_for_cls(rules):
    s = "\"text\"\n"
    for rule in rules:
        s += "\"<s>Human: 给出一条规则，请你将它分类为软件需求相关规则、软件需求无关规则和领域知识之一。\n规则: "
        s += rule['text']
        s += "\n</s><s>Assistant: "
        if rule['type'] == "0":
            s += "软件需求无关规则"
...
if __name__ == "__main__":
    rules = json.load(open("../data/rule_extraction_train_base.json", "r", encoding="utf-8"))
    s = generate_llm_chat_data_for_ir_v1(rules)
    with open("../data/rule_extraction_llama2_train.csv", "w", encoding="utf-8") as f:
        f.write(s)
    
    rules = json.load(open("../data/rule_extraction_validate.json", "r", encoding="utf-8"))
    s = generate_llm_chat_data_for_ir_v1(rules)
    with open("../data/rule_extraction_llama2_validate.csv", "w", encoding="utf-8") as f:
        f.write(s)
    
    rules = json.load(open("../data/rule_filtering_train_base.json", "r", encoding="utf-8"))
    s = generate_llm_chat_data_for_cls(rules)
    with open("../data/rule_filtering_llama2_train.csv", "w", encoding="utf-8") as f:
        f.write(s)
    
    rules = json.load(open("../data/rule_filtering_validate.json", "r", encoding="utf-8"))
    s = generate_llm_chat_data_for_cls(rules)
    with open("../data/rule_filtering_llama2_validate.csv", "w", encoding="utf-8") as f:
        f.write(s)